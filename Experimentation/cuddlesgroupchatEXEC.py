...
def initiate_group_chat(scan_results, tools_used):
    config_list = [
    {
        "model": "gpt-4o",
        "api_key": os.getenv("OPENAI_API_KEY"),
    ...
    pentest_strategist = autogen.AssistantAgent(
        name="PentestStrategist",
        system_message="Strategic in penetration testing and security analysis.",
        llm_config=llm_config,
    )
    groupchat = autogen.GroupChat(agents=[user_proxy, security_analyst, pentest_strategist], messages=[], max_round=10)
...
def main():
    ...
    # Initiate group chat to discuss the results and strategize the next steps
    initiate_group_chat(scan_result, tools_used)
